...
// notificações push — só usada dentro do app nativo (iOS/Android via
// Capacitor); no navegador normal isso nunca chega a ser chamado (ver
// initNativePush() mais abaixo)
...
// app nativo. Melhor esforço: qualquer falha aqui fica só no console, nunca
// atrapalha o login/uso normal do jogo (mesmo padrão do sendPushToUser no
// servidor, ver functions/index.js).
...
// (capturado de ?ref= na URL — ver linha ~2652). Veio de link: trava o campo
// (não dá pra "roubar" a indicação de outra pessoa trocando o valor). Não
// veio de link: deixa livre pra digitar o código de um amigo manualmente.
...
window.showMenu = () => {
  ...
  if (offline) {
    ...
    labelEl.innerHTML = '';
    labelEl.insertAdjacentHTML('beforeend', avatarOrDefaultIcon(equippedAvatar, 18) + ' ');
...
// fica vazio lá — o replay mostra os cliques certos, sem cursor se movendo entre eles.
...
async function fetchScoresFromSnapshot() {
  ...
  const rows = [];
  // o retrato (recomputeScoresSnapshot em functions/index.js) não filtra
  // admin no servidor — é compartilhado com teste.html, que mostra admin de
  // propósito; aqui em produção continua de fora de qualquer ranking. Já
  // banido fica de fora dos dois, teste.html e produção.
  chunkSnaps.forEach(cs => { if (cs.exists()) (cs.data().rows || []).forEach(r => { if (r.data && r.data.admin !== true && r.data.banned !== true) rows.push(r); }); });
  return rows;
}
...
async function fetchScoresLive() {
  ...
  snap.forEach(d => {
    const data = d.data();
    // contas admin (campo definido só pelo Firebase Console) ficam de fora de
    // qualquer ranking, embora tenham acesso liberado a tudo no jogo. Contas
    // banidas também ficam de fora de qualquer ranking.
    if (data.nick && data.admin !== true && data.banned !== true) rows.push({ uid: d.id, data });
  });
  return rows;
}
...
async function renderRankPreview(field, bodyElId, myPts) {
  ...
  try {
    ...
    // localiza (ou insere) a linha do jogador (admin/banido nunca aparece, nem pra si mesma)
    const youLabel = lang === 'en' ? 'YOU' : lang === 'es' ? 'TÚ' : 'VOCÊ';
    let myIndex = (!offline && currentUser) ? rows.findIndex(r => r.uid === currentUser.uid) : -1;
    if (myIndex === -1 && myData.admin !== true && myData.banned !== true) {
...
async function renderProfile(viewStats, viewNick, viewUid) {
  ...
  $('profile-nick-avatar').innerHTML = avatarOrDefaultIcon(stats.equipped && stats.equipped.avatar, 24) + ' ';
...
window.loadRanking = async (m) => {
  ...
  try {
    ...
    // (exceto se for admin ou banido — essas nunca devem aparecer no ranking, nem pra si mesmas)
    if (!offline && currentUser && myData.nick && myData.admin !== true && myData.banned !== true && !rows.some(r => r.uid === currentUser.uid)) {
...
async function loadDailyTodayRanking() {
  ...
  try {
    ...
    snap.forEach(d => {
      ...
      // só entra quem a gente confirma que não é admin nem banido: veio do
      // fetchAllScores (que já exclui os dois) ou, se for a própria pessoa
      // logada, checamos myData direto — assim nenhum admin/banido aparece
      // nesse ranking, nem pra si mesmo
      const stats = statsByUid[data.uid] || (isMe ? myData : null);
      if (!stats || stats.admin === true || stats.banned === true) return;
...
function shopItemPreview(item) {
  ...
  if (item.slot === 'sfxCorrect') {
    ...
    wrap.innerHTML = avatarSvg(item.id, 40);
  }
  return wrap;
}
...
function setPvpNameWithLevel(el, labelText, xp, avatarId) {
  el.innerHTML = '';
  el.insertAdjacentHTML('beforeend', avatarOrDefaultIcon(avatarId, 18) + ' ');
